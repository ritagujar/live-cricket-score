...
class Score extends Component {
  ...
  getScore = (score) => {
    ...
    ans.first =
      ...
      numb[0] &&
      nummid[0] &&
      numb[0].replace(/[^0-9]/g, "") + "/" + nummid[0].replace(/[^0-9]/g, "");
    ans.second =
      numb &&
      nummid &&
      numb[2] &&
      nummid[1] &&
      nummid[1].replace(/[^0-9]/g, "") + "/" + numb[2].replace(/[^0-9]/g, "");
    ...
    return (
      <Card style={{ marginTop: 20, width: 700 }}>
        <Box className={this.props.classes.component}>
          ...
          <Button
            onClick={() => this.getDetails(this.props.match.unique_id)}
            variant="contained"
            size="small"
            color="primary"
            style={{ marginLeft: "auto", border: "1px solid #fff" }}
            disabled={this.props.match.matchStarted ? false : true}
          >
            See Score
          </Button>
        </Box>
        <CardContent>
          <Box className={this.props.classes.container}>
            <Typography className={this.props.classes.date}>
              {new Date(this.props.match.dateTimeGMT).toLocaleString()}
            </Typography>
            <Typography className={this.props.classes.matchDetails}>
  ...
  render() {
    return <React.Fragment>{this.getMatchCart()}</React.Fragment>;
  }
}
